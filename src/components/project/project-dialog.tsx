...
export const ProjectDialog = ({ projectId, isNewProject }: ProjectDialogProps) => {
	const { project, setProject, loading, error, updateProject } = useProject(
		isNewProject ? undefined : projectId,
	);
		...
		<ProjectContext.Provider value={{ project, setProject }}>
			<Card>
				...
				<CardFooter className="flex items-start gap-4">
					...
					<Button id="back" onClick={() => navigate('/projects')} variant="outline">
						Отмена
					</Button>
				</CardFooter>
			</Card>
