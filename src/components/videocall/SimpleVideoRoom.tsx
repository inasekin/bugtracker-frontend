...
export function SimpleVideoRoom({
	...
	useEffect(() => {
		...
		if (room && room.participants && room.participants.length > 0) {
			setParticipants(room.participants.filter((p) => p.userName).map((p) => p.userName));
		}
		...
		try {
			...
			if (err instanceof DOMException) {
				if (err.name === 'NotAllowedError') {
					errorMessage =
						'Доступ к камере и микрофону заблокирован. Пожалуйста, разрешите доступ в настройках браузера и перезагрузите страницу.';
			...
			<VideoRoomHeader
				...
				onScreenShareToggle={() =>
					toast({
						...
						description: 'Демонстрация экрана временно отключена',
					})
				}
				onFullScreenToggle={() =>
					toast({
						title: 'Функция недоступна',
						description: 'Полноэкранный режим временно отключен',
					})
				}
				onShare={shareRoom}
				onLeave={leaveRoom}
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 h-full">
					...
					{participants.length > 0 ? (
						participants.map((name, index) => (
							<div
								key={index}
								className="relative bg-black rounded-lg overflow-hidden flex items-center justify-center text-white"
							>
