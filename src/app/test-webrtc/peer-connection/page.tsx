		...
		<div className="container mx-auto p-4 space-y-6">
			...
			<Card>
				...
				<CardContent>
					...
					<div className="flex flex-wrap gap-2">
						<Button onClick={createOffer} disabled={!cameraStream}>
							Создать предложение (Offer)
						</Button>
						<Button onClick={createAnswer} disabled={!cameraStream || !remoteSDP}>
							Создать ответ (Answer)
						</Button>
						<Button
							onClick={receiveAnswer}
							disabled={!peerConnection || !remoteSDP || !isCallInitiator}
						>
							Принять ответ
						</Button>
						<Button
				...
				<Card>
					...
					<CardContent>
						<div className="relative aspect-video bg-black rounded-lg overflow-hidden">
							...
							<div className="absolute inset-0 flex items-center justify-center text-white">
								{peerConnection
									? 'Ожидание удаленного соединения...'
									: 'Инициализируйте WebRTC'}
							</div>
						</div>
					</CardContent>
				</Card>
			...
			<Card>
				...
				<CardContent className="space-y-4">
					<div>
						<Label htmlFor="localSDP">
							Ваш локальный SDP ({isCallInitiator ? 'Offer' : 'Answer'}):
						</Label>
					...
					<div>
						<Label htmlFor="remoteSDP">
							Введите удаленный SDP ({isCallInitiator ? 'Answer' : 'Offer'}):
						</Label>
						<div className="flex items-center mt-1">
							<Input
								id="remoteSDP"
