...
export class VideoCallApi extends BaseApi {
	...
	async createRoom(
		...
		try {
			...
			throw new Error(
				'Не удалось создать комнату: ' + (error.message || 'Неизвестная ошибка'),
			);
		}
	}
		...
		try {
			...
			if (error.response) {
				...
				if (status === 404) {
					...
					throw new Error(
						'У вас нет прав на присоединение к этой комнате или неверный код доступа',
					);
				} else if (status === 409) {
			...
			throw new Error(
				'Не удалось присоединиться к комнате: ' + (error.message || 'Неизвестная ошибка'),
			);
		}
	}
	...
	async getUserRooms(): Promise<VideoRoom[]> {
		try {
			...
			throw new Error(
				'Не удалось получить список комнат: ' + (error.message || 'Неизвестная ошибка'),
			);
		}
	}
	...
	async getPublicRooms(): Promise<VideoRoom[]> {
		try {
			...
			throw new Error(
				'Не удалось получить список публичных комнат: ' +
					(error.message || 'Неизвестная ошибка'),
			);
		}
	}
	...
	async closeRoom(roomId: string): Promise<void> {
		try {
			...
			throw new Error(
				'Не удалось закрыть комнату: ' + (error.message || 'Неизвестная ошибка'),
			);
		}
	}
	...
	async leaveRoom(roomId: string): Promise<void> {
		try {
			...
			throw new Error(
				'Не удалось выйти из комнаты: ' + (error.message || 'Неизвестная ошибка'),
			);
		}
	}
}
