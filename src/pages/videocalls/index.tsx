...
export function VideoCallsPage() {
	...
	const createRoom = async () => {
		...
		try {
			setIsLoading(true);
			const room = await videoCallApi.createRoom(newRoomName, isPrivate);
			setRooms([...rooms, room]);
			setNewRoomName('');
			setError(null);
		} catch (err) {
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
					...
					<div className="space-y-4">
						...
						<div className="flex items-center space-x-2">
							...
							<Label htmlFor="room-private">
								{isPrivate
									? 'Приватная комната (только для авторизованных)'
				...
				<div className="p-4 bg-white rounded-lg shadow">
					...
					<p className="text-gray-600 mb-4">
						У вас есть код доступа к комнате? Присоединитесь к существующей
						видеоконференции.
					</p>
					<Button variant="outline" onClick={goToJoinByCode} className="w-full">
						Ввести код доступа
					</Button>
				</div>
			</div>
